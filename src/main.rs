...
mod tests {
    ...
    #[test]
    fn test_sbox_lookup() {
        // Test 1: S-Box Lookup für 0x53 sollte 0xed zurückgeben
        let input = 0x53;
        let expected_output = 0xed;
        let result = sbox_lookup(input);
        assert_eq!(result, expected_output, "S-Box lookup for 0x53 failed!");

        // Test 2: S-Box Lookup für 0x09 sollte 0x01 zurückgeben
        let input = 0x09;
        let expected_output = 0x01;
        let result = sbox_lookup(input);
        assert_eq!(result, expected_output, "S-Box lookup for 0x09 failed!");

        // Test 3: S-Box Lookup für 0xcf sollte 0x18 zurückgeben
        let input = 0xcf;
        let expected_output = 0x18;
        let result = sbox_lookup(input);
        assert_eq!(result, expected_output, "S-Box lookup for 0xcf failed!");

        // Test 4: S-Box Lookup für 0x4f sollte 0x9d zurückgeben
        let input = 0x4f;
        let expected_output = 0x9d;
        let result = sbox_lookup(input);
        assert_eq!(result, expected_output, "S-Box lookup for 0x4f failed!");

        // Test 5: S-Box Lookup für 0x3c sollte 0x87 zurückgeben
        let input = 0x3c;
        let expected_output = 0x87;
        let result = sbox_lookup(input);
